...
plotBootstrapDistributions <- function(boot, sets=NULL) {
...
plotBootstrapCI <- function(boot, sets=NULL) {
  if(is.null(sets)) sets <- names(boot)
  P <- lapply(sets, function(set) {
...
plotBootstrapBox <- function(boot) {
...
Here I perform bootstrap analysis with fixed $\tau_2=8$.
...
```{r bootstrap_fit_params_3par, fig.width=8, fig.height=14}
plotBootstrapDistributions(boot.3)
```

And here are fit parameters and their 95% confidence intervals:

```{r bootstrap_fit_params_CI_3par, fig.width=6, fig.height=3}
plotBootstrapCI(boot.3)
```

These are default box plots with whiskers encompassing 90% of data:

```{r bootstrap_fit_params_box_3par, fig.width=6, fig.height=3}
plotBootstrapBox(boot.3)
```
