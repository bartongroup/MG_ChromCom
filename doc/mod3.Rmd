...
fitting.script <- paste0(topDir, "R/fitting.R")
bootstrap.script <- paste0(topDir, "R/bootstrap.R")
...
echr <- lapply(dataFile, function(file) experimentalData(paste0(dataDir, file)))
```
...
for(t0 in c(0, -5, -10, -15)) {
  ...
  cmd <- paste("Rscript", fitting.script, "scramble", name, switch, ncells, ntry, t0, npar)
...
chr.scr <- lapply(neb, function(t0) {
  file <- paste0(binDir, "fitt_scramble_t.", -t0, "_tau1_k1_k2_tau2.rds")
  readRDS(file)
})
...
for(set in sets) {
  ...
  cmd <- paste("Rscript", fitting.script, set, name, switch, ncells, ntry, t0, npar)
  qsub(cmd, logDir, name=name, ncor=8)
}
```
...
cmd <- paste("Rscript", fitting.script, "RAD21", name, switch, ncells, ntry, t0, npar, 15)
qsub(cmd, logDir, name=name, ncor=8)
```

```{r RAD21_tau8_tau15, fig.width=6, fig.height=4}
rad21.tau8 <- readRDS(paste0(binDir, "fits_RAD21_ref1_t0_n10000_tau1_k1_k2.rds"))
rad21.tau15 <- readRDS(paste0(binDir, "fits_RAD21_tau2_15_ref1_t0_n10000_tau1_k1_k2.rds"))
...
runBootstrap <- function(set, nbatch, root="boot", ncells=10000, ntry=30, switch=1, t0=0, npars=4) {
  for(batch in 1:nbatch) {
    ...
    cmd <- paste("Rscript", bootstrap.script, args)
...
```{r four_colour}
echr4 <- lapply(dataFile, function(file) experimentalData(paste0(dataDir, file), map=model.colours.extended))
