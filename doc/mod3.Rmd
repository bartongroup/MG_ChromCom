...
colourMap <- function(echr, condition, window=c(-300,-20), colour="N") {
  ...
  map <- cells == colour
  # important assumption: gaps are non-colour
  map[is.na(map)] <- FALSE
  cmap <- list()
  k <- 1
  for(i in 1:nrow(map)) {
...
plotIntervalDistribution <- function(echr, condition, window, with.theoretical=FALSE) {
  ...
  g <- ggplot(df, aes(dist)) +
    ...
    xlim(0, 40) +
    labs(x="Interval length (min)", y="Density", title=paste(condition, sprintf("%5.3f", lambda)))
...
plotIntervalDistribution(echr4, "scramble", c(-300, -20), with.theoretical = TRUE)
```
...
```{r interval_distributions, fig.width=9, fig.height=9}
P <- list()
for(nm in names(echr4)) {
  P[[nm]] <- plotIntervalDistribution(echr4, nm, c(-300, -20))
}
...
```{r interval_distributions_G2_S, fig.width=6, fig.height=3}
P <- list()
for(nm in names(pchr)) {
  P[[nm]] <- plotIntervalDistribution(pchr, nm, c(-300, 300))
}
grid.arrange(grobs=P, ncol=2) 
```
