...
source("../R/lib.R")
binDir <- "../RData"
fitDir <- "/home/mgierlinski/projects/chromcomR/RData/"
figDir <- "/home/mgierlinski/projects/chromcomR/figures/"

opts_chunk$set(fig.path='../figure/mod3-', 
...
for(t0 in c(0, -5, -10, -15)) {
  name <- paste0("fitt_scramble_t.", -t0, "_tau1_k1_k2_tau2")
  cmd <- paste("Rscript", script, "scramble", name, switch, 10000, 30, t0, 4)
...
chr.scr <- lapply(neb, function(t0) {
  name <- paste0("fitt_scramble_t.", -t0, "_tau1_k1_k2_tau2")
  cacheData(name, fitChr, binDir=fitDir, cacheonly=TRUE)
})
...
```{r fit_all_conditions_4par, eval=FALSE}
script <- "/cluster/gjb_lab/mgierlinski/projects/chromcomR/R/fitting.R"
logDir <- "/cluster/gjb_lab/mgierlinski/projects/chromcomR/log/"
switch <- 1
...
plotAllConditions <- function(suffix) {
  ...
  chr <- lapply(nms, function(set) {
    ...
    fname <- paste0(fitDir, name, ".RData")
    if(file.exists(fname)) {
      cacheData(name, binDir=fitDir, cacheonly=TRUE)
    }
  })
  names(chr) <- nms
  P <- list()
...
```{r fit_all_conditions_3par, eval=FALSE}
script <- "/cluster/gjb_lab/mgierlinski/projects/chromcomR/R/fitting.R"
logDir <- "/cluster/gjb_lab/mgierlinski/projects/chromcomR/log/"
switch <- 1
...
P <- plotAllConditions("ref1_t0_n10000_tau1_k1_k2")
grid.arrange(grobs=P, ncol=2)
```
...
```{r fit_RAD21, eval=FALSE}
script <- "/cluster/gjb_lab/mgierlinski/projects/chromcomR/R/fitting.R"
logDir <- "/cluster/gjb_lab/mgierlinski/projects/chromcomR/log/"
switch <- 1

name <- paste0("fits_RAD21_tau2_15_ref1_t0_n10000_tau1_k1_k2")
cmd <- paste("Rscript", script, "RAD21", name, switch, 10000, 30, 0, 3, 15)
qsub(cmd, logDir, name=name, ncor = 8)
```

```{r RAD21_tau8_tau15, fig.width=6, fig.height=4}
rad21.tau8 <- cacheData("fits_RAD21_ref1_t0_n10000_tau1_k1_k2", binDir=fitDir, cacheonly=TRUE)
rad21.tau15 <- cacheData("fits_RAD21_tau2_15_ref1_t0_n10000_tau1_k1_k2", binDir=fitDir, cacheonly=TRUE)
...
runBootstrap <- function(set, nbatch, root="boot", ncells=10000, ntry=30, switch=1, t0=0, npars=4) {
  for(batch in 1:nbatch) {
    args <- paste(set, root, batch, switch, ncells, ntry, t0, npars)
    cmd <- paste("Rscript", script, args)
    qsub(cmd, logDir, name=paste0(root, "_", set, "_", batch), ncor=8, host="login-gjb-1.compbio.dundee.ac.uk", queue="c6100.q,64bit-pri.q")
  }
}
...
bootParamStats <- function(dat, pars=c("tau1", "tau2", "k1", "k2")) {
  df <- NULL
  for(par in pars) {
...
bootParamPlot <- function(dat, title="", pars=c("tau1", "tau2", "k1", "k2")) {
...
readBootstraps <- function(sets, nboot=300, root="boot") {
  boot <- lapply(sets, function(set) {
    readBootstrap(set, nboot, root=root)
  })
  names(boot) <- sets
  boot
}

bootstrapParamsTab <- function(boot, sets=NULL, pars=c("tau1", "tau2", "k1", "k2"), format="%.3g") {
...
plotBootstrapDistributions <- function(boot, sets=NULL, pars=c("tau1", "tau2", "k1", "k2")) {
  if(is.null(sets)) sets <- names(boot)
  P <- lapply(sets, function(set) {
...
plotBootstrapParamCor <- function(boot, sets=NULL, pars=c("tau2", "k2")) {
  if(is.null(sets)) sets <- names(boot)
...
plotBootstrapCI <- function(boot, sets=NULL, pars=c("tau1", "tau2", "k1", "k2")) {
  if(is.null(sets)) sets <- names(boot)
  P <- lapply(sets, function(set) {
...
plotBootstrapBox <- function(boot, pars=c("tau1", "tau2", "k1", "k2")) {
  md <- plyr::ldply(boot, .id="set")
  md <- md[,c("set", pars)]
...
runBootstrap("untreated", nboot)

runBootstrap("scramble", nboot)
runBootstrap("NCAPD2", nboot)
runBootstrap("NCAPD3", nboot)
runBootstrap("SMC2", nboot)

runBootstrap("RAD21", nboot)
runBootstrap("WAPL48", nboot)
runBootstrap("TT108", nboot)

runBootstrap("MK1775", nboot)
runBootstrap("MK1775_ICRF193", nboot)
```

```{r read_bootstraps}
sets <- c("untreated", "scramble", "NCAPD2", "NCAPD3", "SMC2", "RAD21", "WAPL48", "TT108")
boot <- readBootstraps(sets, nboot=nboot, root="boot") 
```
...
```{r bootstrap_run_3par, eval=FALSE}
runBootstrap("untreated", nboot, root="boot_par3", npars=3)

runBootstrap("scramble", nboot, root="boot_par3", npars=3)
runBootstrap("NCAPD2", nboot, root="boot_par3", npars=3)
runBootstrap("NCAPD3", nboot, root="boot_par3", npars=3)
runBootstrap("SMC2", nboot, root="boot_par3", npars=3)

runBootstrap("RAD21", nboot, root="boot_par3", npars=3)
runBootstrap("WAPL48", nboot, root="boot_par3", npars=3)
runBootstrap("TT108", nboot, root="boot_par3", npars=3)
```


```{r read_bootstraps_3par}
sets <- c("untreated", "scramble", "NCAPD2", "NCAPD3", "SMC2", "RAD21", "WAPL48", "TT108")
boot.3 <- readBootstraps(sets, nboot=nboot, root="boot_par3") 
```

Here is the distribution of bootstrap result. Each bootstrap gives a set of fit parameters. Figures below show the distribution of fit parameters across all bootstraps.

```{r bootstrap_fit_params_3par, fig.width=6, fig.height=14}
plotBootstrapDistributions(boot.3, pars=c("tau1", "k1", "k2"))
```

And here are fit parameters and their 95% confidence intervals:

```{r bootstrap_fit_params_CI_3par, fig.width=5, fig.height=3}
plotBootstrapCI(boot.3, pars=c("tau1", "k1", "k2"))
```

These are default box plots with whiskers encompassing 90% of data:

```{r bootstrap_fit_params_box_3par, fig.width=5, fig.height=3}
plotBootstrapBox(boot.3, pars=c("tau1", "k1", "k2"))
```

In this table we show the median value of each parameter:

```{r bootstrap_fit_params_median_3par}
df <- bootstrapParamsTab(boot.3, pars=c("tau1", "k1", "k2"))
kable(df, format="html", row.names=FALSE) %>% kable_styling("condensed", full_width = FALSE, position="left", font_size=12)
```
...
colourMap <- function(echr, condition, window=c(-300,-20)) {
  ch <- echr[[condition]]
  cells <- ch$cells
...
All figures are in the [this folder](http://www.compbio.dundee.ac.uk/user/mgierlinski/chromcom/figures).
...
plotDataModel <- function(set, boot.pars) {
 ...
 plotTimelines(chr, expdata = echr$untreated, withpars=FALSE, title=set)
}
```
...
```{r fig_four_colour}
echr4 <- lapply(dataFile, function(file) experimentalData(file, map=model.colours.extended))
P <- lapply(sets, function(nm) plotCells(echr4[[nm]], palette=c("blue", "chocolate4", "pink", "red")) + labs(title=nm))
pdf(paste0(figDir, "cell_tracks.pdf"), width=4, height=14)
grid.arrange(grobs=P, ncol=1)
dev.off() 
```
