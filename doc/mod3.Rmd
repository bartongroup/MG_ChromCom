...
- Time $t_1$ is a random variable with exponential distirbution with time scale $\tau_1$
- Time $\Delta t_2$ is a random variable with exponential distirbution with time scale $\tau_2$
- Time $\Delta t_3$ is a random variable with exponential distirbution with time scale $\tau_3$
...
\* A switch, $t_{2, ref}$ was introduced to the model, selecting the reference for time $t_2$. In the default position ($t_{2, ref} = 1$) the B$\rightarrow$P activation time is $t_0-t_1 + \Delta t_2$. When the switch is set to $t_{2, ref} = 0$, the activation time is $t_0 + \Delta t_2$, that is, it can only occur after the nuclear envelope breakdown. The idea of the switch was to separate pink and red curves, as observed in some datasets.
...
Somehow, the wrong versio was used to do fitting and pars$t2ref is a string. Need to convert (only once).
...
```{r fig_all_conditions,fig.width=5}
for(name in names(dataFile)) {
  print(plotTimelines(chr[[name]], expdata = echr[[name]], withpars=TRUE, title=name, title.size=9))
}
```



## Switch: P$\rightarrow$R only after zero
