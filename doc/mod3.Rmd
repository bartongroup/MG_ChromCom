...
public_html <- "http://www.compbio.dundee.ac.uk/user/mgierlinski/aphids/"
...
bootParamStats <- function(dat, pars=c("HL1", "HL2", "k1", "k2")) {
  ...
  for(par in pars) {
    ...
    #q <- signif(q, 3)
    #q <- sprintf("%.2g", q)
    row <- data.frame(par=par, median=q[3], lo=q[1], up=q[5], iqr=q[4] - q[2])
    df <- rbind(df, row)
  }
  df
}
...
bootstrapParamsTabCI <- function(boot, sets=NULL, pars=c("HL1", "HL2", "k1", "k2"), format="%.3g") {
  ...
  P <- lapply(sets, function(set) {
    ...
    c(
      sprintf(format, bp$median),
      sprintf(format, bp$lo),
      sprintf(format, bp$up),
      sprintf(format, bp$iqr)
    )
  })
  names(P) <- sets
  df <- plyr::ldply(P, .id="set")
  colnames(df) <- c("set", paste0(pars, "_med"), paste0(pars, "_lo"), paste0(pars, "_up"), paste0(pars, "_iqr"))
  df
}
...
In this table we show the median, lower and upper 95% CI and the interquartile range (iqr) of each parameter:
...
remoteDir <- "/home/mgierlinski/projects/chromcom/tables/"
public_html <- "http://www.compbio.dundee.ac.uk/user/mgierlinski/chromcom/tables/"
write.table(df, file=paste0(remoteDir, file), sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE)
link <- paste0(public_html, file) 
```
