...
# in.bins=TRUE : don't use time information, distribution based on data bins only
...
plotIntervalDistribution <- function(echr, condition, window, with.theoretical=FALSE, what="interarrival") {
...
Here I calculate the density of brown boxes and the distribution of interarrival times between them. If brown points are distributed randomly with density $\lambda$, the intervals between them should follow the exponential distribution with PDF

$f(d) = \lambda e^{-\lambda d}$

where $d$ is the interarrival time between consecutive brown points. Because some of the cells have much shorter data tracks, they will bias the interarrival distribution towards shorter intervals. The longer distances are missing from these short tracks. To account for this I made a simple model, in which I take cell tracks from the actual data, fill them randomly with brown points and calculate interarrival distribution. This is done 100 times to smooth the distribution.
...
However, this is not what we really want. Interarrival time between individual one-minute boxes assumes that each box is one event. But in reality, brown event can last less or more than one minute. If we see, e.g., four brown boxes next to each other, it is unlikely that this shows four one-minute events. Much more likely, this is one event that lasts four minutes. Hence, considering the distribution of times between one-minute boxes and comparing it to a Poisson distribution of random one-minute events does not make much sense.

# Brown event duration and gap distribution

Instead, we should consider the distribution of full brown events, short and long. Below, I show the distribution of durations of brown events.
...
NOTE: the blue open bars show the simulated data (see details below). There is a slight difference between real and simulated data. This is because for each simulation (which is repeated 100 times) gapes in real data are filled with random data. Hence, the simulated data shows the most likely real data, with imputation.

But we can get more insight from the distribution of intervals between the brown events. Here I use the gaps between the events, not taking brown begin/end into account. For example, if "N" is brown and "B" is blue then this cell track

```
BBBNBBNNNNBBBNB
```

shows three brown events of length 1, 4, and 1 and two "gap" events of lenghts 2 and 3. The blue sequences at the beginning and at the end are ignored, because we don't know how long they are, we only see part of them.
...
- The process is repeated 100 times and the mean distirbution of randomized data is whown

If brown events were random, both data and simulated distributions would be similar. The p-value in the title shows the result from a K-S test comparing data and simulation. The number "d" in each title is the density of brown boxes.
...
Now, I do K-S test between each pair of distirbutions.

```{r brown_distribution_KS_function}
getDist <- function(condition, window, what) {
...
ksMap <- function(what) {
  ...
  for(i in 1:(n-1)) {
    for(j in (i+1):n) {
      cond1 <- names(echr4)[i]
      cond2 <- names(echr4)[j]
      dist1 <- getDist(cond1, window, what)
      dist2 <- getDist(cond2, window, what)
...
### KS test for event duration distribution

```{r brown_distribution_KS_fills}
ksMap("fills")
```

### KS test for gaps between events

```{r brown_distribution_KS_gaps}
ksMap("gaps")
```

These tables show that there is very little discernable difference between conditions. Only TT103 differs from some other conditions in gap distribution.
...
for(nm in names(pchr)) {
  P[[k]] <- plotIntervalDistribution(pchr, nm, c(-300, 300), what="fills")
  P[[k+1]] <- plotIntervalDistribution(pchr, nm, c(-300, 300), what="gaps")
  k <- k + 2
}
grid.arrange(grobs=P, ncol=2)
```
