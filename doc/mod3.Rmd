...
First, I allow for $t_1$, $k_1$ and $k_2$ to be free. $\Delta t_2$, $\Delta t_3$ and $k_3$ are fixed at zero. Then, I repeat the fit with $\Delta t_2$ allowed to be free. The three-parameter model result is on the left, the four-parameter model is on the right.
...
As above, for NCAPD2 data.

```{r, fig.width=10}
echr.n2 <- experimentalData(dataFile$NCAPD2)
pars <- c3pars()
chr3.n2 <- cacheData("fit_NCAPD2_n3000_3par", fitChr, echr.n2, pars, npar=3, nsim=3000, ntry=16, binDir=binDir)
g1 <- plotTimelines(chr3.n2, expdata = echr.n2, withpars=TRUE)

pars <- c3pars(dt2=5)
chr4.n2 <- cacheData("fit_NCAPD2_n3000_4par", fitChr, echr.n2, pars, npar=4, nsim=3000, ntry=16, binDir=binDir)
g2 <- plotTimelines(chr4.n2, expdata = echr.n2, withpars=TRUE)
grid.arrange(g1, g2, ncol=2)
```

Strangely, the four-parameter model has worse RMS. This is probably because the fitting procedure doesn't cope very well with stochastic models. I might have to redo this fit with more simulations.
...
```{r, fig.width=10}
echr.n3<- experimentalData(dataFile$NCAPD3)
pars <- c3pars()
chr3.n3 <- cacheData("fit_NCAPD3_n3000_3par", fitChr, echr.n3, pars, npar=3, nsim=3000, ntry=16, binDir=binDir)
g1 <- plotTimelines(chr3.n3, expdata = echr.n3, withpars=TRUE)

pars <- c3pars(dt2=10)
chr4.n3 <- cacheData("fit_NCAPD3_n3000_4par", fitChr, echr.n3, pars, npar=4, nsim=3000, ntry=16, binDir=binDir)
g2 <- plotTimelines(chr4.n3, expdata = echr.n3, withpars=TRUE)
grid.arrange(g1, g2, ncol=2)
```
