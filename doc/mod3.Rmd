...
runBootstrap <- function(set, nbatch, root="boot", ncells=10000, ntry=30, switch=1, t0=0) {
  for(batch in 1:nbatch) {
    args <- paste(set, batch, switch, ncells, ntry, t0)
...
Here are the median, lower and upper bound on 95% confidence intervals and bootstrap distributions of each of the fit parameters. Note that confidence intervals are quite large!

```{r bootstrap_run_scramble, eval=FALSE}
runBootstrap("scramble", 300)
```

```{r bootstrap_results_scramble, fig.width=5, fig.height=4}
boot.scramble <- readBootstrap("scramble", 300)

bootParamStats(boot.scramble)
bootParamPlot(boot.scramble)
```

### NCAPD2

```{r bootstrap_run_NCAPD2, eval=FALSE}
runBootstrap("NCAPD2", 300)
```

```{r bootstrap_results_NCAPD2, fig.width=5, fig.height=4}
boot.NCAPD2 <- readBootstrap("NCAPD2", 300)

bootParamStats(boot.NCAPD2)
bootParamPlot(boot.NCAPD2)
```

### NCAPD3

```{r bootstrap_run_NCAPD3, eval=FALSE}
runBootstrap("NCAPD3", 300)
```

```{r bootstrap_results_NCAPD3, fig.width=5, fig.height=4}
#boot.NCAPD3 <- readBootstrap("NCAPD3", 300)

#bootParamStats(boot.NCAPD3)
#bootParamPlot(boot.NCAPD3)
```
...
```{r chi2_test}
contingency.row <- function(d, max.count=30) {
  d <- d[d <= max.count]
  d <- tapply(d, d, length)
  x <- rep(0, max.count)
...
chi2.simul <- function(dist, rdist, max.count=30) {
  x <- contingency.row(dist, max.count)
  p <- contingency.row(rdist, max.count)
  p[p == 0] <- 0.5
  p <- p / sum(p)
  chi <- chisq.test(x, p=p)
}

chi2.data <- function(dist1, dist2, max.count=30) {
  x <- contingency.row(dist1, max.count)
  y <- contingency.row(dist2, max.count)
