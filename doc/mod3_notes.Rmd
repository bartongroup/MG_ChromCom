...
fitChr <- function(echr, pars, npar=3, nsim=1000, ntry=10) {
  chr <- ChromCom3(pars)
  ...
  for(i in 1:ntry) {
    ...
    print()
    opt <- optim(p, errorFun, gr=NULL, echr, nsim, method="L-BFGS-B", lower=lower, upper=upper, control=list(trace=3))
    if(opt$value < chi2) {
      chi2 <- opt$value
      p <- opt$par
    }
...
```{r, fig.width=5}
pars <- c3pars()
chr <- cacheData("fit_n1000_3par", fitChr, echr, pars, npar=3, nsim=1000, binDir=binDir)
...
```{r, fig.width=5}
pars <- c3pars(dt2=5)
chr <- cacheData("fit_n1000_4par", fitChr, echr, pars, npar=4, nsim=1000, binDir=binDir)
plotTimelines(chr, expdata = echr, withpars=TRUE)
```
