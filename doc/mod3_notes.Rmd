...
However, I'm not sure if this is exacly the model I use in the simulation. In the simulation, we assumed that P->R transition can happen only after the B->P transition. 
...
Finally I used "L-BFGS-B" method which allows box constraints.
...
errorFun <- function(p, echr, nsim=1000) {
  pars <- vectorPar(p)
...
```{r, fig.width=5}
startpars <- c(-30, 0.05, 0.05)
lower <- c(-50,0,0)
upper <- c(0,0.2,0.2)
opt <- cacheData("optim_n1000_3par", optim, startpars, errorFun, gr=NULL, echr, method="L-BFGS-B", lower=lower, upper=upper, control=list(trace=3), binDir=binDir)

pars <- vectorPar(opt$par)
chr <- ChromCom3(pars, timepars=list(start=-90, stop=30, step=1))
chr <- generateCells(chr, nsim=1000, method="simulation")
...
```{r}
startpars <- c(-30, 0.05, 0.05, 5)
lower <- c(-50, 0, 0, 0)
upper <- c(0, 0.2, 0.2, 30)

opt <- cacheData("optim_n1000_4par", optim, startpars, errorFun, gr=NULL, echr, method="L-BFGS-B", lower=lower, upper=upper, control=list(trace=3), binDir=binDir, refresh=TRUE)
